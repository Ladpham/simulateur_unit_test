...
/* Typography */
h1 { font-size: 1.25rem !important; font-weight: 800 !important; letter-spacing: -0.03em; margin-bottom: 0 !important; }
...
# ==================================================
    ...
    with tab_sim:
        ...
        with inp_col:
            ...
            with r1c1:
                st.markdown('<div class="slider-row-label">Revenus / trx</div>', unsafe_allow_html=True)
                st.slider("rev", 1.0, 7.0, step=0.01, key="revenu_pct", label_visibility="collapsed",
                          help="Take-rate effectif sur montant décaissé. Réel Fév : 4.58%")
                st.markdown('<div class="slider-hint">Réel Fév : 4.58%</div>', unsafe_allow_html=True)

                st.markdown('<div class="slider-row-label" style="margin-top:8px;">Coût liquidité (10j)</div>', unsafe_allow_html=True)
                st.slider("liq", 0.0, 1.5, step=0.01, key="cout_liquidite_10j_pct", label_visibility="collapsed",
                          help=f"Coût capital 10j. Annualisé : {taux_liquidite_annuel_pct:.1f}%")
                st.markdown(f'<div class="slider-hint">Annualisé : {taux_liquidite_annuel_pct:.1f}% — réel : 0.30%</div>', unsafe_allow_html=True)

            with r1c2:
                st.markdown('<div class="slider-row-label">Coût paiement</div>', unsafe_allow_html=True)
                st.slider("pay", 0.0, 3.0, step=0.01, key="cout_paiement_pct", label_visibility="collapsed",
                          help="Rails paiement Visa/MC/MoMo. Open Banking → 0.50%")
                st.markdown('<div class="slider-hint">OB cible : 0.50% — réel : 1.75%</div>', unsafe_allow_html=True)

                st.markdown('<div class="slider-row-label" style="margin-top:8px;">Défaut 30j (PAR30)</div>', unsafe_allow_html=True)
                st.slider("dft", 0.0, 5.0, step=0.01, key="defaut_30j_pct", label_visibility="collapsed",
                          help="Perte nette sur portefeuille. WariBrain → cible <0.80%")
                st.markdown('<div class="slider-hint">WariBrain cible : 0.80% — réel : 1.50%</div>', unsafe_allow_html=True)
            ...
            with v1:
                st.markdown('<div class="slider-row-label">Net loan portfolio (k€)</div>', unsafe_allow_html=True)
                st.slider("lb", 10.0, 5000.0, step=5.0, key="loan_book_k", label_visibility="collapsed")
                st.markdown(f'<div class="slider-hint">Réel Fév : 135k€ — valeur : {loan_book_k:,.0f} k€</div>', unsafe_allow_html=True)
            with v2:
                st.markdown('<div class="slider-row-label">Cycles / mois</div>', unsafe_allow_html=True)
                st.slider("cyc", 1.0, 4.0, step=0.01, key="cycles_per_month", label_visibility="collapsed")
                st.markdown(f'<div class="slider-hint">Réel Fév : 2.63 — valeur : {cycles_per_month:.2f}</div>', unsafe_allow_html=True)
            ...
            # Card 3: Par prêt + Opex
            st.markdown('<div class="wb-card">', unsafe_allow_html=True)
            st.markdown('<div class="section-label">Par prêt &amp; coûts fixes</div>', unsafe_allow_html=True)
            p1, p2, p3, p4 = st.columns(4, gap="small")
            with p1:
                st.markdown('<div class="slider-row-label">Prêt moy. (€)</div>', unsafe_allow_html=True)
                st.slider("al", 100.0, 1000.0, step=10.0, key="avg_loan_value_eur", label_visibility="collapsed")
                st.markdown(f'<div class="slider-hint">{avg_loan_value_eur:,.0f} € — réel 338€</div>', unsafe_allow_html=True)
            with p2:
                st.markdown('<div class="slider-row-label">Prêts / retailer</div>', unsafe_allow_html=True)
                st.slider("txc", 1.0, 8.0, step=0.1, key="tx_per_client_per_month", label_visibility="collapsed")
                st.markdown(f'<div class="slider-hint">{tx_per_client_per_month:.1f} — réel 2.6</div>', unsafe_allow_html=True)
            with p3:
                st.markdown('<div class="slider-row-label">Opex (k€/mois)</div>', unsafe_allow_html=True)
                st.slider("opx", 0.0, 200.0, step=1.0, key="opex_current_k", label_visibility="collapsed")
                st.markdown(f'<div class="slider-hint">{opex_current_k:,.0f} k€ — réel 12k</div>', unsafe_allow_html=True)
            with p4:
                st.markdown('<div class="slider-row-label">COGS / retailer (€)</div>', unsafe_allow_html=True)
                st.slider("cog", 0.0, 50.0, step=0.5, key="cogs_per_client_eur", label_visibility="collapsed")
                st.markdown(f'<div class="slider-hint">{cogs_per_client_eur:.1f} €</div>', unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
    ...
    with tab_pnl:
        ...
        with tbl_col:
            ...
            # Extra P&L inputs
            st.markdown("")
            st.markdown('<div class="section-label">Hypothèses P&L</div>', unsafe_allow_html=True)
            pe1, pe2 = st.columns(2)
            with pe1:
                st.markdown('<div class="slider-row-label">CAC / new retailer (€)</div>', unsafe_allow_html=True)
                st.slider("cac", 0.0, 200.0, step=1.0, key="cac_per_new_client_eur", label_visibility="collapsed")
            with pe2:
                st.markdown('<div class="slider-row-label">Horizon croissance (mois)</div>', unsafe_allow_html=True)
                st.markdown(f'<div class="slider-hint">Base : {base_clients:.0f} retailers actifs</div>', unsafe_allow_html=True)
                st.slider("gm", 1, 36, step=1, key="growth_months", label_visibility="collapsed")
...
#             # Waterfall chart
#             st.markdown('<div class="section-label">Décomposition par transaction</div>', unsafe_allow_html=True)
#             wf_steps = ["Revenu", "Paiement", "Liquidité", "Défaut 30j", "CM nette"]
#             wf_values = [revenu_pct, -cout_paiement_pct, -cout_liquidite_10j_pct, -defaut_30j_pct, contribution_margin_pct]
#             wf_starts, wf_ends = [], []
#             running = 0.0
...
#             wf_starts.append(0.0)
#             wf_ends.append(contribution_margin_pct)
#             wf_types = ["positive", "negative", "negative", "negative", "total"]

#             wf_df = pd.DataFrame({
#                 "step": wf_steps, "value": wf_values,
#                 "start": wf_starts, "end": wf_ends, "type": wf_types,
#             })
#             cs = alt.Scale(domain=["positive", "negative", "total"], range=["#1B5A43", "#F83131", "#064C72"])
#             wf_bar = (
#                 alt.Chart(wf_df).mark_bar(cornerRadiusTopLeft=4, cornerRadiusTopRight=4,
#                                           cornerRadiusBottomLeft=4, cornerRadiusBottomRight=4)
#                 .encode(
#                     x=alt.X("step:N", sort=wf_steps, title=None, axis=alt.Axis(labelFontSize=11)),
#                     y=alt.Y("start:Q", axis=alt.Axis(title="%", labelFontSize=10)),
#                     y2="end:Q",
#                     color=alt.Color("type:N", scale=cs, legend=None),
#                     tooltip=[alt.Tooltip("step:N"), alt.Tooltip("value:Q", format=".2f", title="%")],
#                 )
#             )
#             wf_lbl = (
#                 alt.Chart(wf_df).mark_text(dy=-9, fontSize=11, fontWeight="bold", color="#333")
#                 .encode(
#                     x=alt.X("step:N", sort=wf_steps),
#                     y="end:Q",
#                     text=alt.Text("value:Q", format="+.2f"),
...
#             # Revenue breakdown
#             st.markdown('<div class="section-label">Revenus détaillés</div>', unsafe_allow_html=True)
#             rm1, rm2, rm3, rm4 = st.columns(4)
#             with rm1:
#                 st.metric("MRR", f"{monthly_revenue_eur/1000:.1f} k€")
#             with rm2:
#                 st.metric("Revenue / an", f"{annual_revenue_eur/1000:.0f} k€")
#             with rm3:
...
#                 bar_pct = min(100, ratio * 100)
#                 bar_color = "#1B5A43" if ratio >= 1.0 else "#064C72"
#                 st.markdown('<div class="section-label">Progression vers le break-even</div>', unsafe_allow_html=True)
#                 st.markdown(f"""
#                 <div style="background:rgba(0,0,0,0.05); border-radius:8px; height:10px; overflow:hidden;">
...
#                   <span><strong>{loan_book_k:,.0f} k€</strong> net book actuel</span>
#                   <span style="opacity:0.6;">cible break-even : <strong>{be_book_k:,.0f} k€</strong></span>
#                 </div>
#                 """, unsafe_allow_html=True)
#             else:
#                 st.markdown(
#                     '<div style="font-size:11px; color:#C0392B; padding:8px 12px; '
#                     'background:rgba(192,57,43,0.06); border-radius:8px;">'
#                     '⚠️ CM négative — ajuste les curseurs de coûts</div>',
#                     unsafe_allow_html=True,
#                 )
